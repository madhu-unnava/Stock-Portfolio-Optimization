...
#C:/Users/deepak.nayak/Downloads/ISDS570/Project/daily_prices_2015_2020.csv
dp<-read.csv('C:/Users/deepak.nayak/Downloads/ISDS570/Project/daily_prices_2015_2020.csv') # no arguments
#Explore
...
eod_ret<-tail(eod_ret,-1) #use tail with a negative value #to remove the NAs
#check
eod_ret[1:10,1:3] #first 10 rows and first 3 columns 
ncol(eod_ret)
nrow(eod_ret)
...
#used to return annualized return graph untill 2020 Dec
eod_ret_subset <- head(eod_ret, n = nrow(eod_ret) - 58)

# Selected tickers
selected_tickers <- c("JAZZ", "JPM.PD", "JXHLY", "NVZMF", "NILSY", "NKSH", "CCBC", "CRWS", "CME", "USLM", "UNB", "USNU", "SQBG", "STLD", "SEB")

# Extract data for the first five rows and selected tickers
selected_data <- eod_ret[1:5, c(selected_tickers)]
...
random12 <- c("JAZZ","JPM.PD","JXHLY","NVZMF","NILSY","NKSH","CCBC","CRWS","CME","USLM","UNB","USNU","SQBG","STLD","SEB")
# We need to convert data frames to xts (extensible time series)
#Ra for getting annualized return chart till Mar 2021
Ra<-as.xts(eod_ret[,random12,drop=F])
Rb<-as.xts(eod_ret[,'SP500TR',drop=F]) #benchmark
# Rc for getting annualized return chart till Dec 2020
Rc<-as.xts(eod_ret_subset[,random12,drop=F])
head(Ra)
head(Rb)
...
# Returns
table.AnnualizedReturns(cbind(Rb,Ra),scale=58) # note for monthly use scale=12
...
# Plot cumulative returns with adjusted legend size

chart.CumReturns(Ra, legend.loc = 'topleft', main = "Cumulative Returns", cex.legend = 0.57)
chart.CumReturns(Rb,legend.loc = 'topleft')
#cumulative ret chart for date till Dec 2020
chart.CumReturns(Rc, legend.loc = 'topleft', main = "Cumulative Returns", cex.legend = 0.57)

combined_matrix <- cbind(Rb, Ra)

# Display the first 5 rows
cbindres <- head(combined_matrix, 5)
print(cbindres)
